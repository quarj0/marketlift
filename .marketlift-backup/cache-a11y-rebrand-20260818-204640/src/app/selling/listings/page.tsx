          ...
          <section className="min-w-0 overflow-hidden rounded-2xl border bg-white shadow-sm" aria-labelledby="listing-manager-title">
            ...
            <div className="marketlift-scrollbar overflow-x-auto border-b px-3 py-3 sm:px-4">
              <div className="flex min-w-max gap-2" role="tablist" aria-label={t('selling.listings.filter')}>
                {filters.map((item) => (
                  <button key={item} type="button" role="tab" aria-selected={filter === item} onClick={() => setFilter(item)} className={`min-h-10 rounded-full px-4 py-2 text-xs font-black transition ${filter === item ? 'bg-brand-700 text-white shadow-sm' : 'bg-slate-100 text-slate-600 hover:bg-brand-50 hover:text-brand-800'}`}>
                    {filterLabel(item)}
                  </button>
                ))}
              </div>
