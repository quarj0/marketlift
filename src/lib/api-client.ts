...
export class MarketliftApiError extends Error {
  ...
  constructor(message: string, extensions?: GraphQLErrorExtensions) {
    ...
    this.status = typeof extensions?.status === 'number' ? extensions.status : undefined;
...
export async function ensureCsrfToken() {
  ...
  if (!csrfPromise) {
    csrfPromise = fetch(resolveApiUrl('/api/v1/auth/csrf/'), {
      ...
      .then(async (response) => {
        if (!response.ok) throw new MarketliftApiError('Unable to initialize a secure session.', { status: response.status });
...
export async function apiRequest<T>(
  ...
  if (csrf || (rest.method && rest.method !== 'GET' && rest.method !== 'HEAD')) {
  ...
  if (!response.ok) {
    ...
    throw new MarketliftApiError(message, { status: response.status, details: body });
  }
  return body as T;
}
...
export async function graphqlRequest<T>(query: string, variables: Record<string, unknown> = {}): Promise<T> {
  ...
  if (typeof document === 'undefined') {
    ...
    if (!csrfResponse.ok) throw new MarketliftApiError('Unable to initialize a secure API request.', { status: csrfResponse.status });
  ...
  if (!response.ok) {
    throw new MarketliftApiError('GraphQL request failed.', { status: response.status, details: payload });
  }
