...
export function ListingCard({
  ...
  return (
    <>
      <article className={`group overflow-hidden rounded-2xl border bg-white shadow-sm transition hover:border-slate-300 hover:shadow-md ${list ? 'sm:grid sm:grid-cols-[240px_1fr]' : ''}`}>
        <div className={`relative overflow-hidden bg-slate-100 ${list ? 'aspect-16/10 sm:aspect-auto sm:min-h-44' : 'aspect-4/3'}`}>
          <Link
          ...
          >
            <Image
              ...
              sizes={list ? '(min-width:640px) 240px, 100vw' : '(max-width:640px) 50vw, 25vw'}
            />
          </Link>
          ...
          <button
            ...
            aria-label={saved ? t('listing.unsave', { title: listing.title }) : t('listing.save', { title: listing.title })}
            aria-pressed={saved}
        ...
        <div className="p-3 sm:p-4">
          ...
          {list && <p className="mt-2 line-clamp-2 text-sm text-slate-500">{listing.description}</p>}
