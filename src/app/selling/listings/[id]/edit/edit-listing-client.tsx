  ...
  const mutation = useMutation({
    mutationFn: () => {
      ...
      return sellingService.updateListing(listing.id, {
        ...
        condition: categoryQuery.data.condition.enabled
          ? form.condition
          : undefined,
      ...
      <main className="mx-auto max-w-7xl px-4 py-5 pb-28 sm:px-6 sm:py-8 lg:px-8 lg:pb-10">
        ...
        <div className="grid min-w-0 gap-4 sm:gap-6 lg:grid-cols-[230px_minmax(0,1fr)]">
          ...
          {listingQuery.isLoading ? (
            <section className="grid min-h-72 place-items-center rounded-2xl border bg-white">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <label className="sm:col-span-2">
                  ...
                  <textarea
                    ...
                    className="min-h-40 w-full rounded-xl border p-3 text-sm outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-100"
                  />
                </label>
              ...
              <div className="mt-8 border-t pt-6">
                    ...
                    <div className="mt-4 flex gap-3 overflow-x-auto pb-2">
                      {listing.images.map((src, index) => (
                        <Image
                          ...
                          width={112}
                          height={84}
                          className="h-24 w-28 shrink-0 rounded-xl border object-cover"
                        />
                      ))}
                    </div>
                  )}
                <label className="mt-4 flex min-h-24 cursor-pointer items-center justify-center gap-3 rounded-xl border-2 border-dashed p-4 text-sm font-semibold text-slate-600 hover:border-brand-400 hover:bg-brand-50/30">
                  ...
                  {replacementPhotos.length
                    ? `${replacementPhotos.length} ${t("selling.new.step.photos")}`
                    : t("selling.new.choosePhotos")}
                  <input
                    type="file"
