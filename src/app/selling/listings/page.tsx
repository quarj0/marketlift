...
export default function SellerListings() {
  ...
  const filterLabel = (value: Filter) =>
    value === "all" ? t("selling.listings.all") : t(`selling.status.${value}`);
    ...
    <MarketplaceShell>
      <main className="mx-auto max-w-7xl px-4 py-5 pb-28 sm:px-6 sm:py-8 lg:px-8 lg:pb-10">
        <div className="mb-5 flex flex-col gap-4 sm:mb-7 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-xs font-black uppercase tracking-[.14em] text-brand-700 sm:text-sm">
              {t("selling.eyebrow")}
            </p>
            <h1 className="mt-1 text-2xl font-black tracking-tight sm:text-3xl">
              {t("selling.listings.title")}
            </h1>
            <p className="mt-1 text-sm leading-6 text-slate-500 sm:text-base">
              {t("selling.listings.manageBody")}
            </p>
          </div>
          ...
          >
              ...
              >
                {filters.map((item) => (
                  <button
                    ...
                    className={`min-h-11 rounded-full px-4 py-2 text-xs font-black transition ${filter === item ? "bg-brand-700 text-white shadow-sm" : "bg-slate-100 text-slate-600 hover:bg-brand-50 hover:text-brand-800"}`}
                  >
                    {filterLabel(item)}
                  </button>
                ))}
            ...
            {query.isLoading && (
              <div
                ...
                aria-busy="true"
                aria-label={t("selling.listings.loading")}
              >
                {[1, 2, 3].map((item) => (
                  <div
                    key={item}
            ...
            {query.isError && (
              <div className="px-5 py-14 text-center" role="alert">
                ...
                <h3 className="mt-3 font-black">
                  {t("selling.listings.loadError")}
                </h3>
                <p className="mt-1 text-sm text-slate-500">
                  {t("selling.listings.loadErrorBody")}
                </p>
                <Button
                  type="button"
                  variant="outline"
            ...
            {!query.isLoading && !query.isError && rows?.length === 0 && (
              <div className="px-5 py-14 text-center">
                <h3 className="font-black">
                  {t("selling.listings.emptyStatus")}
                </h3>
                <p className="mx-auto mt-1 max-w-sm text-sm text-slate-500">
                  {t("selling.listings.emptyBody")}
                </p>
                {filter !== "all" && (
                  <Button
                    type="button"
                  ...
                  <article key={listing.id} className="p-4 sm:p-5">
                    <div className="grid min-w-0 grid-cols-[96px_minmax(0,1fr)] gap-3 sm:grid-cols-[104px_minmax(0,1fr)_auto] sm:items-center sm:gap-4">
                      ...
                      <div className="col-span-2 mt-1 grid grid-cols-3 gap-2 sm:col-span-1 sm:mt-0 sm:flex sm:flex-wrap sm:justify-end">
                        ...
                        <Button
                          ...
                          title={
                            !paymentsEnabled ? t("common.upcoming") : undefined
                          }
                        ...
                        <Button
                          ...
                          loadingText={
                            listing.status === "paused"
                              ? t("selling.listings.resuming")
                              : t("selling.listings.pausing")
                          }
                          disabled={deleteBusy}
                          onClick={() =>
                        ...
                        >
                          ...
                          {listing.status === "paused"
                            ? t("selling.listings.resume")
                            : t("selling.listings.pause")}
                        </Button>
                        <Button
                          size="sm"
                          variant="ghost"
                    ...
                    {confirmDelete === listing.id && (
                      <div
                        ...
                        role="alertdialog"
                        aria-label={t("selling.listings.deleteLabel", {
                          title: listing.title,
                        })}
                      >
                        <p className="text-sm font-bold text-rose-900">
                          {t("selling.listings.confirmDelete")}
                        </p>
                        <p className="mt-1 text-xs leading-5 text-rose-700">
                          {t("selling.listings.deleteBody")}
                        </p>
                        <div className="mt-3 flex gap-2">
                          ...
                          >
                            {t("selling.listings.deleteListing")}
                          </Button>
                          <Button
                            type="button"
                            size="sm"
