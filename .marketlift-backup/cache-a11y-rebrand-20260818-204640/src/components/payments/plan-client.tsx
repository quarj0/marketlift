    ...
    <div className="min-w-0 space-y-5 sm:space-y-6">
      <div className="flex flex-col gap-4 rounded-2xl border bg-white p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between sm:p-5">
        ...
        <div className="grid grid-cols-2 rounded-xl bg-slate-100 p-1" aria-label={t('payments.plan.billing')}>
          <button type="button" aria-pressed={cycle === 'monthly'} onClick={() => setCycle('monthly')} className={`min-h-10 rounded-lg px-4 py-2 text-sm font-bold ${cycle === 'monthly' ? 'bg-white shadow-sm' : 'text-slate-600'}`}>{t('payments.plan.monthly')}</button>
          <button type="button" aria-pressed={cycle === 'yearly'} onClick={() => setCycle('yearly')} className={`min-h-10 rounded-lg px-4 py-2 text-sm font-bold ${cycle === 'yearly' ? 'bg-white shadow-sm' : 'text-slate-600'}`}>{t('payments.plan.yearly')} <span className="text-brand-700">{t('payments.plan.save')}</span></button>
        </div>
      </div>
