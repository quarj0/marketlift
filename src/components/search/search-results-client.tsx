    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          ...
          <select id="search-filter-state" value={filters.state} onChange={(event) => update({ state: event.target.value, city: undefined })} className="h-11 w-full rounded-xl border bg-white px-3 text-sm">
        ...
        <div>
          ...
          <select id="search-filter-city" disabled={!filters.state} value={filters.city} onChange={(event) => update({ city: event.target.value })} className="h-11 w-full rounded-xl border bg-white px-3 text-sm disabled:bg-slate-50">
      ...
      <div>
        ...
        <Input id="search-filter-neighborhood" defaultValue={filters.district} key={`d-${filters.district}`} placeholder={t('selling.new.districtPlaceholder')} onKeyDown={(event) => { if (event.key === 'Enter') update({ district: event.currentTarget.value }); }} />
      </div>
      <fieldset>
