...
import type { AccountOverview, AccountProfile, AccountReview, AccountSettings } from '@/types';
...
let myReviews: AccountReview[] = [
  {
    ...
    rating: 5,
    comment: 'Clear communication and the vehicle matched the description. The seller was punctual and professional.',
    date: '2026-08-10',
    sellerReply: 'Thanks, Lucas. It was a pleasure doing business with you!',
  ...
  },
  {
    ...
    reviewerName: 'Lucas Martins',
    rating: 4,
    comment: 'Fast responses and everything was as described. Pickup was easy to arrange.',
    date: '2026-07-22',
...
export const accountService = {
  async getOverview(): Promise<AccountOverview> {
    ...
    return {
      ...
      savedListings: [listings[0], listings[2], listings[6]],
    };
  },

  async getSaved() {
    await delay();
    return [listings[0], listings[2], listings[6], listings[8]];
  },
  ...
  async updateProfile(input: Partial<AccountProfile>): Promise<AccountProfile> {
    ...
    profile = {
      ...profile,
      ...input,
      location: input.location ? { ...profile.location, ...input.location } : profile.location,
    };
    return { ...profile, location: { ...profile.location } };
  },
