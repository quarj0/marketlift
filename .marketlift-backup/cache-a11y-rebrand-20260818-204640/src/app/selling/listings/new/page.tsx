                      ...
                      {photos.map((photo, index) => (
                        <div key={photo.url} className={`relative aspect-square overflow-hidden rounded-xl border ${cover === index ? 'ring-2 ring-brand-500' : ''}`}>
                          ...
                          <div className="absolute inset-x-0 bottom-0 flex items-center justify-between bg-black/60 p-2 text-white">
                            <button type="button" className="min-h-9 px-1 text-xs font-bold" onClick={() => setCover(index)}>{cover === index ? t('selling.new.cover') : t('selling.new.setCover')}</button>
                            <button type="button" className="grid size-9 place-items-center rounded-lg hover:bg-white/15" onClick={() => removePhoto(index)} aria-label={t('selling.new.removePhoto', { name: photo.name })}><Trash2 className="size-4" /></button>
                          </div>
