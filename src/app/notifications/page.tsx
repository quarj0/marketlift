              ...
              >
                ...
                <div className="flex-1">
                  <div className="flex justify-between gap-4">
                    <p className="font-semibold">{t(`notifications.${notification.id}.title`)}</p>
                    <span className="text-xs text-slate-400">{tr(notification.createdAt)}</span>
                  </div>
                  <p className="mt-1 text-sm text-slate-500">{t(`notifications.${notification.id}.body`)}</p>
                </div>
                {!notification.read && <span className="mt-2 size-2 rounded-full bg-brand-600" />}
              </Link>
            ))
          ) : (
            <div className="p-10 text-center text-sm text-slate-500">{t('notifications.empty')}</div>
          )}
        </div>
      </main>
    </MarketplaceShell>
