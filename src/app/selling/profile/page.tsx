          ...
          <div className="space-y-6">
            {loading ? (
              <section className="grid min-h-64 place-items-center rounded-2xl border bg-white shadow-sm">
                <Loader2 className="size-6 animate-spin text-brand-600" aria-label={t('common.loading')} />
              </section>
            ) : !seller || !profile ? (
              <section className="rounded-2xl border border-rose-200 bg-rose-50 p-6 text-rose-800">
                <p className="font-semibold">{t('common.error')}</p>
                <Button variant="outline" className="mt-4" onClick={() => { void profileQuery.refetch(); void sellerQuery.refetch(); }}>
                  {t('common.tryAgain')}
                </Button>
              </section>
            ) : (
                  ...
                  <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
                    <div className="relative w-fit">
                      ...
                      <label className="absolute -bottom-2 -right-2 grid size-11 cursor-pointer place-items-center rounded-full border bg-white shadow focus-within:ring-2 focus-within:ring-brand-500">
                        <span className="sr-only">{t('selling.profile.photo')}</span>
                    ...
                    <div>
                      ...
                      <Button variant="outline" size="sm" className="mt-3" asChild>
                        <Link href={`/seller/${seller.id}`}><Eye className="size-4" />{t('selling.profile.preview')}</Link>
                      </Button>
                    </div>
                  </div>
                  {avatarMutation.isError && <p className="mt-4 text-sm font-semibold text-rose-600">{t('common.error')}</p>}
                </section>
                ...
                <section className="rounded-2xl border bg-white p-5 shadow-sm sm:p-6">
                  ...
                  <div className="mt-5 grid gap-4 sm:grid-cols-2">
                    <label className="sm:col-span-2">
                      <span className="mb-1.5 block text-sm font-bold">{t('selling.profile.displayName')}</span>
                      <Input value={form.displayName} onChange={(event) => setForm((current) => ({ ...current, displayName: event.target.value }))} />
                    </label>
                    <label>
                      <span className="mb-1.5 block text-sm font-bold">{t('selling.profile.type')}</span>
                      <select
                        className="h-11 w-full rounded-xl border bg-white px-3 text-sm"
                        value={form.sellerType}
                        onChange={(event) => setForm((current) => ({ ...current, sellerType: event.target.value as SellerType }))}
                      >
                        <option value="individual">{t('selling.profile.individual')}</option>
                        <option value="business">{t('selling.profile.business')}</option>
                      </select>
                    </label>
                    <label>
                      <span className="mb-1.5 block text-sm font-bold">{t('selling.profile.phone')}</span>
                    ...
                    <label className="sm:col-span-2">
                      <span className="mb-1.5 block text-sm font-bold">{t('selling.profile.about')}</span>
                      <textarea
                        value={form.bio}
                        maxLength={240}
                  ...
                  {saveMutation.isError && <p className="mt-4 rounded-xl bg-rose-50 p-3 text-sm font-semibold text-rose-700">{t('common.error')}</p>}
                  <Button
                    className="mt-5 w-full sm:w-auto"
                    disabled={saveMutation.isPending || !form.displayName.trim() || !form.city.trim()}
