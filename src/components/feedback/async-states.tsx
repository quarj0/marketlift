  ...
  return (
    <div className="grid min-h-56 place-items-center rounded-2xl border bg-white p-8" role="status" aria-live="polite">
      <div className="text-center">
        <LoaderCircle className="mx-auto size-7 animate-spin text-brand-600" aria-hidden="true" />
        <p className="mt-3 text-sm font-semibold text-slate-700">{label ?? t('common.loadingContent')}</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="rounded-2xl border border-rose-200 bg-rose-50 p-5" role="alert">
      <div className="flex gap-3">
        ...
        <div>
          <p className="font-bold text-rose-950">{title ?? t('common.error')}</p>
          <p className="mt-1 text-sm leading-6 text-rose-800">{description ?? t('common.errorBody')}</p>
          {onRetry && (
            <Button variant="outline" className="mt-4 border-rose-200 bg-white" onClick={onRetry}>
              {t('common.tryAgain')}
            </Button>
          )}
        </div>
