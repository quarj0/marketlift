...
function useFilters(): SearchFilters {
  ...
  const legacyMatch = legacyLocation.match(/^(.*?)(?:,|\s)\s*([A-Za-z]{2})\s*$/);
  const legacyState = legacyMatch
    ? brazilLocations.find((state) => state.code === legacyMatch[2].toUpperCase())
    : undefined;
  const state = params.get('state') || legacyState?.code || '';
  const city = params.get('city') || (legacyState ? legacyMatch?.[1].trim() || '' : '');
...
export function SearchResultsClient() {
  ...
  const [cityDraftState, setCityDraftState] = useState<{ key: string; value: string } | null>(null);
  const [districtDraftState, setDistrictDraftState] = useState<{ key: string; value: string } | null>(null);
  const cityDraft = cityDraftState?.key === cityFilterKey
    ? cityDraftState.value
    : filters.city || '';
  const districtDraft = districtDraftState?.key === districtFilterKey
    ? districtDraftState.value
    : filters.district || '';
  const setCityDraft = (value: string) => setCityDraftState({ key: cityFilterKey, value });
  const setDistrictDraft = (value: string) => setDistrictDraftState({ key: districtFilterKey, value });
  ...
  const cities = citiesQuery.data?.length
    ...
    : [...(selectedState?.cities ?? [])].filter((city) =>
        city.toLocaleLowerCase('pt-BR').includes(cityDraft.toLocaleLowerCase('pt-BR')),
      );
  const neighborhoods = neighborhoodsQuery.data ?? [];

  function update(patch: Record<string, string | undefined>) {
    const next = new URLSearchParams(searchParams.toString());
    const changesManualLocation = ['region', 'state', 'city', 'district', 'location'].some(
    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        <select
          ...
          onChange={(event) => { setCityDraft(''); setDistrictDraft(''); update({ region: event.target.value, state: undefined, city: undefined, district: undefined, location: undefined }); }}
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <div>
          ...
          <Input
            ...
            onBlur={() => {
              setDistrictDraft('');
              update({ city: cityDraft.trim() || undefined, district: undefined, location: undefined });
            }}
            onKeyDown={(event) => {
              if (event.key === 'Enter') {
          ...
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <select aria-label={t('search.sortLabel')} value={filters.sort} onChange={(event) => update({ sort: event.target.value })} className="h-11 rounded-xl border bg-white px-3 text-sm">
                ...
                {hasCoordinates && <option value="distance">{t('search.sort.distance')}</option>}
