...
export function AuthShell({
  ...
  return (
    <main id="main-content" className="min-h-screen bg-slate-50 lg:grid lg:grid-cols-[minmax(0,1fr)_minmax(420px,.85fr)]">
      <section className="relative hidden overflow-hidden bg-brand-900 p-10 text-white lg:flex lg:min-h-screen lg:flex-col lg:justify-between xl:p-14">
        <div className="absolute -right-20 top-20 size-72 rounded-full bg-brand-400/20 blur-3xl" aria-hidden="true" />
        <div className="absolute -bottom-24 left-20 size-80 rounded-full bg-emerald-300/10 blur-3xl" aria-hidden="true" />
      ...
      <section className="flex min-h-screen flex-col">
        <div className="bg-brand-900 px-4 pb-8 pt-5 lg:hidden">
          <div className="mx-auto flex max-w-xl items-center justify-between gap-2">
            <MarketliftLogo inverse />

            <div className="flex items-center gap-2">
              <LanguageSwitcher compact inverse />
              <Link href="/" className="rounded-xl border border-white/15 px-3 py-2 text-sm font-bold text-white">
                {t('auth.browse')}
              </Link>
            </div>
          </div>
